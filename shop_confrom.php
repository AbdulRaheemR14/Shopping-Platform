...
if (isset($_POST['confirm_payment'])) {
    ...
    $insert_query = "INSERT INTO `user_product` (order_id, invoice_number, amount, product_mode) VALUES (?, ?, ?, ?)";
    $stmt = mysqli_prepare($con, $insert_query);
    mysqli_stmt_bind_param($stmt, "issi", $order_id, $invoice_number, $amount, $payment_mode);
    $data_query = mysqli_stmt_execute($stmt);

    if ($data_query) {
        echo "<h3>Successfully Completed the payment</h3>";
        echo "<script>window.open('shop.php?list_order','_self')</script>";
